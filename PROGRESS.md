...
## Текущая фаза
Фаза 5: Таски и интеграция с Telegram-ботом — done (с одной оговоркой, см.
"Что проверить в первую очередь" и Decisions ниже).
Следующая: Фаза 6 — Роли и доступ (RBAC).
...
- Реальный Telegram end-to-end не проверен (нет BOT_TOKEN) — см. блок в самом
  верху файла с инструкцией, что сделать founder'у вручную.
- Frontend `/tasks` показывает только дашборд просрочек — нет формы создания
  таски из веба (создание пока только через API/будущий bot-flow founder'а);
  не блокер для phase 5 acceptance, но стоит добавить в фазе 6 вместе с RBAC
  UI.

## С чего продолжить следующую сессию
Фаза 6 (`BUILD_PHASES.md` раздел "Фаза 6 — Роли и доступ (RBAC)"): полноценные
permission-мидлвари по ролям на все эндпоинты фаз 1-5 (сейчас — упрощённая
модель `is_founder OR owner/assignee == me`), student-портал (только свои
таски, read-only, вход через Telegram deep-link), manager видит только свои
лиды + назначенные проекты, developer — назначенные проекты/таски. Особо
внимательно пройтись по каждому эндпоинту предыдущих фаз — не оставить дыр.
