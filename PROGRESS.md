...
## Архив сотрудников (2026-07-09)

Founder запросил возможность архивировать сотрудников (не hard delete — по hard
constraint #2 soft delete везде, но `deleted_at` уже занят под другой смысл в
остальной схеме и не показывается в UI как обратимое состояние). Сделано по
готовому паттерну "архив лида" (миграция `202607080009`, `/leads?archived=`) —
то же самое для `users`.

- Миграция `202607090003`: `users.archived_at TIMESTAMPTZ`, `events.entity_type`
  CHECK расширен значением `'user'`.
- `backend/app/api/users.py`: `GET /users?archived=true|false` (default false —
  активная команда). `POST /users/{id}/archive` и `/unarchive`, оба
  founder-only. Архивация нельзя применить к роли `founder` (400) — тривиальная
  защита от ситуации "архивировали единственного founder'а и некому
  разархивировать". Внутри одной транзакции с `FOR UPDATE`: `archived_at =
  now()` + `is_active = false` при архивации; при разархивации — только
  `archived_at = NULL`, **`is_active` НЕ восстанавливается автоматически**
  (founder должен отдельно нажать "Включить"). Осознанное решение — не
  задокументировано отдельно на этапе кода, но проверено живьём и это не баг:
  восстановленный сотрудник не должен тихо получить обратно активный логин без
  явного шага founder'а. Событие `archived`/`unarchived` в `events` с
  `actor_id` founder'а.
- Frontend: `frontend/app/team/page.tsx` — кнопка "Архивировать" на карточке
  (скрыта для founder-карточек), ссылка "Архив сотрудников". Новая
  `frontend/app/team/archive/page.tsx` — список архивированных с датой
  архивации и кнопкой "Восстановить".

**Self-check:** `alembic downgrade base && upgrade head` — чистый цикл (и в
docker-контейнере, и в нативном venv/Postgres). Backend: 5 новых тестов
(`backend/tests/test_user_archive.py`) зелёные, полный набор 64 passed / 4
failed — все 4 падения предсуществующие и не связаны с этой веткой
(перепроверено через `git stash`: те же 4 теста падают и на чистом `dev` до
этих изменений; причина не в этой фиче). `ruff check` чист. Frontend:
`tsc --noEmit` чист. Живая проверка через claude-in-chrome (временный
founder+manager тестовые аккаунты, удалены после проверки): архивация с
карточки в `/team` → сотрудник пропадает из активного списка → появляется в
`/team/archive` с датой → "Восстановить" возвращает его в `/team` (в
неактивном состоянии, ожидаемо).

## Гранулярные права доступа для сотрудников + карточки в "Команде" + фикс интро-анимации логина (2026-07-09)

Founder явно запросил три отдельных фикса/фичи в одной сессии. Все сделаны и
проверены живьём через docker-compose + claude-in-chrome.

**1. Гранулярные permission-флаги (выходит за рамки CRM_SPEC.md раздела 6).**
CRM_SPEC.md раздел 6 описывает только role-based видимость (founder видит
всё, manager — свои лиды/проекты). Founder явно попросил дать возможность
точечно выдавать конкретному сотруднику доступ к тому, что обычно
founder-only, галочками на странице команды. Решение: три nullable-независимых
булевых флага на `users` — `can_view_all_leads`, `can_view_analytics`,
`can_view_finance` (миграция `202607090002`, default `false`). Не стал делать
полноценную permission-таблицу (many-to-many "юзер-право") — на 3 конкретных
права это была бы преждевременная абстракция; если список прав вырастет,
это осознанный повод для рефакторинга в отдельную таблицу, не сейчас.

- `app/core/deps.py`: `CurrentUser` расширен тремя полями (читаются в
  `get_current_user` вместе с `role`). Новые dependency-хелперы
  `require_analytics_access`/`require_finance_access` — пропускают founder
  ИЛИ non-student с соответствующим флагом (student исключён defense-in-depth
  даже если бы флаг был выставлен вручную в БД — UI никогда не предлагает эти
  чекбоксы студенту, но зависимость всё равно проверяет роль).
- `app/api/analytics.py` — все 8 эндпоинтов (`funnel`, `conversion-by-source`,
  `loss-reasons`, `revenue`, `team-load`, `manager-performance`,
  `leads-by-channel-over-time`, `stale-leads`) переведены с `require_founder`
  на `require_analytics_access`.
- `app/api/finance.py` — только 3 founder-only агрегатных эндпоинта
  (`/finance/summary`, `/finance/cash-flow`, `/finance/expenses-by-category`)
  переведены на `require_finance_access`. CRUD над `finance_entries`
  (создание/патч конкретных invoice/payment/expense) остался на
  `require_staff_role` — это не то, что чекбокс "Финансы" призван открывать,
  им и так может пользоваться любой не-student по своим проектам.
- `app/api/leads.py`: `GET /leads` — условие видимости для non-founder стало
  `if user.role != "founder" and not user.can_view_all_leads`, т.е. флаг
  снимает фильтр "только свои + неразобранная очередь" из фазы 6, не трогая
  сам SQL-паттерн (`owner_id IS NULL OR owner_id = $me`).
- `app/api/users.py`: `USER_FIELDS`/`UserIn`/`UserPatch`/insert/update SQL —
  три новых поля, читаются и пишутся так же, как `role_title`.
- Тесты: `backend/tests/test_user_permissions.py`, 5 новых (manager без
  флагов получает 403 и на analytics, и на finance; `can_view_analytics`
  открывает аналитику но не финансы и наоборот; student с флагами,
  выставленными напрямую в БД, всё равно получает 403; `can_view_all_leads`
  снимает owner-фильтр в `GET /leads`). Полный набор: 59/59 (было 54, +5).
  4 существующих упавших теста (`test_convert_won_lead_creates_client_and_project`
  и связанные) — **не мои**, воспроизводятся и на чистом `dev` до этой сессии
  (проверено через `git stash`), не трогал — не связаны с этим изменением.

**2. `/team` — карточки вместо таблицы + фильтр по должности.**
`frontend/app/team/page.tsx` переписан: каждый сотрудник — отдельная
`<article>`-карточка (аватар-инициал, роль, редактируемые поля,
блок "Доступ") вместо строки таблицы. Фильтр — пилюли по уникальным
должностям (`role_title` если заполнен, иначе label роли:
"Founder"/"Менеджер"/"Разработчик"/"Ученик"), с счётчиком в скобках, включая
"Все (N)". Founder-карточка вместо чекбоксов показывает
"Founder видит всё по умолчанию" — чекбоксы доступа рендерятся только для
non-founder. Проверено живьём: чекбоксы у "Зиедулла" (manager) сохраняются
в БД через PATCH `/users/{id}` и переживают reload страницы.

**3. Интро-анимация логина — три репортнутых бага, один настоящий, один
косметический, один архитектурный.**

- **"Пропустить" не работал" — реальная причина оказалась не в коде кнопки.**
  Кнопка (`onClick={onDone}`) и её CSS (z-index/pointer-events) были в
  порядке всё это время — воспроизвести баг через код не удалось. Настоящая
  причина всплыла при живой проверке: контейнер `frontend` держал битый
  webpack dev-cache (`ENOENT .../server-development/*.pack.gz`), из-за чего
  `/login` отдавался как unstyled HTML без JS-чанков (`main-app.js` 404) —
  в таком состоянии ни один обработчик клика на странице не срабатывает,
  не только у кнопки Skip. Фикс — тот же, что уже описан в PROGRESS.md выше
  ("после npm install..."): `docker compose rm -sfv frontend && docker
  compose up -d --build frontend` очищает анонимный volume и пересобирает
  cache с нуля. **Урок:** если на этой машине после серии docker-пересборок
  фронтенда что-то "просто не работает" без видимой причины в коде — первым
  делом смотреть `docker compose logs frontend` на ENOENT/pack.gz, а не
  чинить код, которого не ломали.
- **Текст "опущен вниз" — реальный CSS-баг.** `.login-intro` был
  `display:grid; place-items:center`, а `.intro-steps` (нижняя пилюля с
  шагами) — `position:absolute; bottom:76px`, независимо от контента карточки
  сверху. На вьюпортах, где карточка вместе с закреплённой снизу пилюлей не
  помещались симметрично, композиция визуально съезжала вниз. Исправлено:
  `.login-intro` теперь `display:flex; flex-direction:column;
  align-items:center; justify-content:center; gap:30px`, а `.intro-steps`
  убран из `position:absolute` — обычный flow-элемент в той же колонке, что
  и `.intro-core`. Центрирование теперь одного блока целиком, а не двух
  независимо позиционированных.
- **"Пара шариков" → несколько десятков.** Было 3 хардкоженных
  `.intro-packet` с ручными `--x`/`--y`. Заменено на 34 `.intro-particle`,
  сгенерированных в JSX через `Array.from` по формуле phyllotaxis (золотой
  угол 137.508°) — **без `Math.random()`**, т.к. это client-компонент,
  рендерящийся и на сервере (SSR) и на клиенте; `Math.random()` в рендере
  дал бы hydration mismatch между SSR и CSR разметкой, что в React может
  сломать обработчики событий на всём поддереве — ровно то, что выглядело бы
  как "кнопка Skip не работает", только по-настоящему, если бы не было
  замечено на этапе разработки. Кривая анимации смягчена
  (`cubic-bezier(.22,1,.36,1)` вместо резкой `.7,0,.22,1`). Стрелки в
  `.intro-steps` получили лёгкую пульсацию потока (`intro-arrow-flow`,
  staggered delay по индексу) для ощущения "движения", не только статичного
  списка шагов.

**4. "Страница загрузки" на 100vh.** В кодовой базе нет отдельного
`loading.tsx`/спиннера — единственный экран, который по смыслу является
"страницей загрузки", это сам `.login-intro`. Добавлено явное
`height: 100vh; height: 100dvh;` поверх существующего `position:fixed;
inset:0`, вторая строка — фоллбэк на динамический viewport height для
мобильных браузеров с уезжающей адресной строкой.

**Побочная находка, не блокер, но важно для следующих сессий на этой
машине:** нативный `npm run dev`/`npm run build` (без Docker) зависает
намертво на этой машине с системным Node v24.14.1 — процесс слушает порт,
но 0% CPU и ни одного ответа на HTTP-запрос даже спустя минуты. Диагностика
заняла много времени (проверялись: сеть, Google Fonts, зомби-процессы),
причина, скорее всего, — несовместимость нативного `@next/swc`-бинарника
Next.js 14.2.35 с Node 24 (Next 14 официально таргетит Node 18-20). Docker
это не задевает, т.к. `frontend/Dockerfile` фиксирует свою версию Node
образа. **Для локальной разработки/проверки на этой машине — только через
`docker compose up`, не через голый `npm run dev`.** Стоит завести отдельный
`.nvmrc`/задокументировать в CLAUDE.md, если это будет мешать чаще одного
раза.
